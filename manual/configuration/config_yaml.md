...
Besides [`.env`](./environment_variables.md) for general settings, the `config.yaml` is used to specify the detailed configurations of the all components in SeaTicket docker.

## File structure of config.yaml

`config.yaml` consists of the following sections (i.e., `global` and name of each component):
...
In cluster mode, you need to specify the URLs used for internal communication between components via `config.yaml`:
...
!!! warning "To ensure the SeaTicket service functions normally, please ensure that `config.yaml` contains **at least one valid LLM**"

For LLMs configurations, you should configure a list option `LLM_MODELS` in `config.yaml`:
...
=== "Other"
    Seafile AI utilizes [LiteLLM](https://docs.litellm.ai/docs/) to interact with LLM services. For a complete list of supported LLM providers, please refer to [this documentation](https://docs.litellm.ai/docs/providers). Then fill the following fields in your `config.yaml`:
...
!!! warning "To ensure the SeaTicket service functions normally, please ensure that `config.yaml` has specified embedding model"
